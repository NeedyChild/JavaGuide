...
- B 树的叶子节点都是独立的;B+树的叶子节点有一条引用链指向与它相邻的叶子节点（ChatGPT的描述：B+树的叶子节点之间以指针相连，形成一个有序链表）。
- B 树的检索的过程相当于对范围内的每个节点的关键字做二分查找，可能还没有到达叶子节点，检索就结束了。而 B+树的检索效率就很稳定了，任何查找都是从根节点到叶子节点的过程，叶子节点的顺序检索很明显。
- 在 B 树中进行范围查询时，首先找到要查找的下限，然后对 B 树进行中序遍历，直到找到查找的上限；而 B+树的范围查询，只需要对（叶子节点的）链表进行遍历即可。

PS(ChatGPT): B树和B+树的范围查询具体来说是这样的：
B树：假设我们要查询范围[15, 25]内的所有关键字。首先，我们找到15，这可能在某个中间节点中。然后，我们执行中序遍历，直到关键字大于25。
B+树：对于同样的查询，在找到15之后（总是在叶子节点），我们可以简单地沿着叶子节点的链表移动，直到关键字大于25。这个过程更高效，因为叶子节点在物理上通过指针连接，使得范围查询更快。
...
**聚簇索引（Clustered Index）即索引结构和数据一起存放的索引，并不是一种单独的索引类型。InnoDB 中的主键索引就属于聚簇索引。**（PS(ChatGPT):在聚簇索引中，叶子节点直接包含了表中的行数据。**具体定义就按Java Guide来记，这里补充GPT的说明是为了方便理解！**)
...
**非聚簇索引(Non-Clustered Index)即索引结构和数据分开存放的索引，并不是一种单独的索引类型。二级索引(辅助索引)就属于非聚簇索引。MySQL 的 MyISAM 引擎，不管主键还是非主键，使用的都是非聚簇索引。** （PS(ChatGPT)：非聚簇索引的叶子节点不直接存储数据。相反，它们包含了指向表中实际数据行的指针。**具体定义就按Java Guide来记，这里补充GPT的说明是为了方便理解！**）
...
试想一种情况，用户准备使用 SQL 查询用户名，而用户名字段正好建立了索引。（即要查询的数据字段恰好被用来建立索引，成为索引的key，那么即使value是引用的非聚簇索引，也不需要借助value的引用再次查询以到达存有整个用户数据的table的该用户数据的位置，而是直接返回索引处的key值即可）
