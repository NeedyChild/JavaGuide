...
- `NULL` 会影响聚合函数的结果。例如，`SUM`、`AVG`、`MIN`、`MAX` 等聚合函数会忽略 `NULL` 值。 `COUNT` 的处理方式取决于参数的类型。如果参数是 `*`号(`COUNT(*)`)，则会统计所有的记录数，包括 `NULL` 值；如果参数是某个字段名(`COUNT(列名)`)，则会忽略 `NULL` 值，只统计非空值的个数。
...
InnoDB 提供事务支持，实现了 SQL 标准定义的四个隔离级别，具有提交(commit)和回滚(rollback)事务的能力。并且，InnoDB 默认使用的 REPEATABLE-READ（可重读）隔离级别是可以解决幻读问题发生的（基于 MVCC 和 Next-Key Lock）。
...
PS（ChatGPT）:幻读与不可重复读类似，但它关注的是由于另一个并发事务插入或删除了一些记录，导致一个事务在执行两次相同的查询时返回了不同的行数。幻读主要是由于插入或删除操作导致的数据不一致。幻读重点关注于新记录的插入或现有记录的删除。
...
- 不可重复读的重点是内容修改或者记录减少比如多次读取一条记录发现其中某些记录的值被修改；(这里是JavaGuide原文，也说明了幻读包含数据丢失的情况）
...
- **SERIALIZABLE(可串行化)** ：最高的隔离级别，完全服从 ACID 的隔离级别。所有的事务依次逐个执行，这样事务之间就完全不可能产生干扰，也就是说，该级别可以防止脏读、不可重复读以及幻读。（PS（ChatGPT）：注意：可串行化并不是指一个事务执行时其他事务不能执行。如果两个事务不冲突（即它们访问的数据集不重叠），它们仍然可以并发执行。准确来说是让事务能够串行执行）
...
- **表级锁：** MySQL 中锁定粒度最大的一种锁（全局锁除外），是针对非索引字段加的锁（这是指InnoDB中的），对当前操作的整张表加锁，实现简单，资源消耗也比较少，加锁快，不会出现死锁。不过，触发锁冲突的概率最高，高并发下效率极低。表级锁和存储引擎无关，MyISAM 和 InnoDB 引擎都支持表级锁。
- **行级锁：** MySQL 中锁定粒度最小的一种锁，是 **针对索引字段加的锁***（指innoDB中，只问行级锁和表级锁的区别时不用提这点） ，只针对当前操作的行记录进行加锁。 行级锁能大大减少数据库操作的冲突。其加锁粒度最小，并发度高，但加锁的开销也最大，加锁慢，会出现死锁。行级锁和存储引擎有关，是在存储引擎层面实现的。

结合ChatGPT的总结：

锁定粒度不同：表级锁锁定整个表，而行级锁锁定单个行

性能开销：表级锁的加锁开销小，但并发性能差；行级锁的加锁开销大（需跟踪每一行的锁定状态），但并发性能好（因为只锁定操作的行，所以其他行仍然可以被其他事务访问和修改，提高了系统的并发能力）
...
(声明锁定意图: 意向锁是一种表级锁，用于表示一个事务打算在表的某些行上加锁（不管是共享锁还是排他锁）。它们不阻止对行的访问，而是用于表示接下来事务将在行级别上加锁。

协调行级锁和表级锁: 通过使用意向锁，InnoDB可以快速判断表是否已被其他事务锁定，这有助于优化锁定逻辑和减少锁冲突。当一个事务想要获得表上的排他锁时，它首先检查是否有其他事务持有该表的任何意向锁。如果有，那么这个事务就必须等待，直到这些意向锁被释放。

减少死锁: 意向锁通过提供一个锁定层次结构来帮助防止死锁。在尝试获取行级锁之前，事务必须先获取对应的意向锁。这样，InnoDB就可以通过检查意向锁来避免潜在的锁冲突，从而减少死锁的发生。)


**意向锁是由数据引擎自己维护的，用户无法手动操作意向锁，在为数据行加共享/排他锁之前，InooDB 会先获取该数据行所在在数据表的对应意向锁。**

**意向锁之间是互相兼容的。** 即多个事务可以同时持有一个表的IS锁或IX锁
...
意向锁和共享锁和排它锁互斥（PS:准确说如下图所示，IS锁与表级别的S锁并不互斥，ChatGPT也是这么说的）（这里指的是**表级别**的共享锁和排他锁，意向锁不会与行级的共享锁和排他锁互斥）。
...
PS：ChatGPT：快照读是指读取数据的一份历史快照版本，这个版本是在事务开始时或查询开始时数据库的一致性快照。快照读不会看到其他事务在快照创建后所做的修改，因此它不会阻塞由于其他事务持有的锁（即不会对数据加锁）。
...
PS：ChatGPT及网络资料：当前读是指读取数据的最新版本。这种读取方式会对选定的数据行加锁，防止其他事务对这些行进行修改，直到当前事务完成。
