...
4. **消息队列(Message Queuing)**：消息队列是消息的链表,具有特定的格式,存放在内核中（PS：网上查的也是说内核，还收藏了个腾讯云的讲解博客作为验证）并由消息队列标识符标识。管道和消息队列的通信数据都是先进先出的原则。与管道（无名管道：只存在于内存中的文件；命名管道：存在于实际的磁盘介质或者文件系统）不同的是消息队列存放在内核中，只有在内核重启(即，操作系统重启)或者显式地删除一个消息队列时，该消息队列才会被真正的删除。消息队列可以实现消息的随机查询,消息不一定要以先进先出的次序读取,也可以按消息的类型读取.比 FIFO 更有优势。**消息队列克服了信号承载信息量少，管道只能承载无格式字 节流以及缓冲区大小受限等缺点。**
...
ChatGPT：死锁是指多任务环境下，多个进程或线程间的资源竞争导致的一种僵局。在这种情况下，涉及的每个任务（进程或线程， 后同）都在等待其他任务释放资源，但这些任务中的任何一个都无法向前推进，因为它们各自所需的资源都被其他任务占有，形成了一个循环等待的局面。这导致了程序无法继续执行
