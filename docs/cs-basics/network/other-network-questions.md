...
#### 补充：计算机五层网络，即在TCP/IP四层模型的基础上，没有把物理层和数据链路层合并成网络接口层（不确定的话可以Google看一下以确认）
...
HTTPS：在HTTP的基础上，通过SSL（安全套接字层）或TLS（传输层安全）协议（记不住的话就说一些协议qwq）提供了数据加密传输和身份验证的功能，确保了数据传输的安全性。
