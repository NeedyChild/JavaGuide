...
- 平均情况时间复杂度：O(nlogn)，对于大多数乱序的情况，快速排序的平均性能很好。
- 最坏情况时间复杂度：O(n^2)，当划分操作每次都分得很不平衡，即每次只能减少一个数据的排序量时（比如数组已经是有序的或者是逆序的），快速排序的效率最低。
- 如果采用随机化快速排序（即选择随机一个数作为基准），那么最差时间复杂度也可被视为O(nlogn)，空间复杂度仍为O(logn)～O(n)。

空间复杂度：

O(logn)～O(n)，空间复杂度取决于递归的深度。在最好的情况下（即每次都能将数据均等分割），空间复杂度为O(logn)；在最坏的情况下（即每次划分只能将数据分成两部分，其中一部分为空），空间复杂度为O(n)。
