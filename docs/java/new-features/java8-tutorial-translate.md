...
1. 接口的默认方法（与“接口方法可以有默认实现”是一个意思（可以Google“接口方法可以有默认实现”以验证，Java 8 使我们能够通过使用 `default` 关键字向接口添加非抽象方法实现）

2. 引入lambda表达式。

3. 引入函数式接口（“函数式接口”是指仅仅只包含一个抽象方法,但是可以有多个非抽象方法(也就是上面提到的默认方法)的接口。像这样的接口，可以被隐式转换为 lambda 表达式）

4. 引入Stream接口

5. 引入一些Date API（比如Clock类提供了访问当前日期和时间的方法，使用ZoneId表示时区，LocalTime和LocalDate用于来表示不包含时区信息的时间和日期。）
