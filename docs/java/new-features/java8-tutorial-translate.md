...
1. 接口的默认方法（与“接口方法可以有默认实现”是一个意思？（可以Google“接口方法可以有默认实现”）以验证，Java 8 使我们能够通过使用 `default` 关键字向接口添加非抽象方法实现）Google查询得到的信息：要使用接口默认实现，只需在接口中的方法前加上default关键字即可。 实现类可以选择是否要覆盖默认方法。 如果实现类覆盖了默认方法，那么调用该方法时将使用实现类的自定义实现。 如果实现类没有提供自定义实现，那么将使用接口中的默认方法。
