...
- `sleep()` 是 `Thread` 类的静态方法，`wait()` 则是 `Object` 类的方法。为什么这样设计呢？下一个问题就会聊到。

## 为什么 wait() 方法不定义在 Thread 中？

`wait()` 是让获得对象锁的线程实现等待，会自动释放当前线程占有的对象锁。每个对象（`Object`）都拥有对象锁，既然要释放当前线程占有的对象锁并让其进入 WAITING 状态，自然是要操作对应的对象（`Object`）而非当前的线程（`Thread`）。

GPT解释（其中的第一点就是JavaGuide作者所写的理由的比较好理解的版本）：
`wait()`方法定义在Java的`Object`类中而不是`Thread`类中，这是由于`wait()`方法的工作机制和多线程协调的需求所决定的。以下是几个主要原因：

1. 锁和监视器的概念

在Java中，每个对象都有一个与之关联的监视器（monitor），用于协调多个线程对同一个对象的访问。当线程调用一个对象的`wait()`方法时，它必须已经获得了那个对象的锁。调用`wait()`后，线程会释放该对象的锁，并进入等待状态，直到其他线程在同一个对象上调用`notify()`或`notifyAll()`。

2. 线程间的协调

`wait()`、`notify()`和`notifyAll()`这三个方法是线程间协调的一种机制，允许线程在特定条件下相互通知和等待。这种机制不仅仅与线程自身有关，更多地与线程操作的对象和对象状态有关。

3. 避免对Thread类的过度膨胀

如果将`wait()`和相关的同步方法定义在`Thread`类中，那么每个线程都将具有这些方法。但在实际中，并不是所有线程都需要使用这些同步机制。通过将它们定义在`Object`类中，Java允许任何对象成为同步的目标，同时避免了对`Thread`类的不必要膨胀。

总结：

综上所述，`wait()`方法被定义在`Object`类中，是因为它与对象的锁和监视器紧密相关，适用于对象级别的同步和线程间协调。这种设计使得Java的每个对象都可以作为线程同步的一部分，提供了一种灵活而强大的线程间通信机制。
