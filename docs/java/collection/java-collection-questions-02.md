...
3. 红黑树：从Java 8开始，HashMap在处理“桶”冲突时引入了红黑树。当特定桶中的链表长度超过一定阈值（默认为8）时，链表会被转换为红黑树，以改善在高冲突情况下的性能。相应地，当树的大小减小到一定程度（默认小于6）时，红黑树会被转换回链表。
...
1. **升级Java版本**：在Java 8中，`HashMap`的实现得到了改进，特别是在处理哈希冲突时，链表会在元素数量较多时转换为红黑树，这减少了死循环的风险，但是Java 8中的HashMap在多线程下仍有数据覆盖等问题。
