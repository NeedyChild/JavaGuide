...
- **底层数据结构：** `ArrayList` 底层使用的是 **`Object` 数组 （PS：而不是可变数组！这个篇幅前面讲ArrayList插入时如果yoa扩容时不是改变数组大小，而是把原有数组复制到一个更大的新数组中去！）**；`LinkedList` 底层使用的是 **双向链表** 数据结构（JDK1.6 之前为循环链表，JDK1.7 取消了循环。注意双向链表和双向循环链表的区别，下面有介绍到！）
- **插入和删除是否受元素位置的影响：**
...
-
- GPT的解释：
- 无序性指的是集合中元素的排列顺序不是固定的，即元素的存放位置没有特定的规则，也不保证每次遍历时元素的顺序都一样。这并不意味着元素是随机排列的，而是它们的排列顺序不受控制，可能由集合内部的数据结构（如哈希表）决定。
- 无序性：这里的“无序”并不意味着“随机”，而是意味着元素的存放位置不遵循可预测的排列顺序。在无序集合中，元素的位置可能是由复杂的算法或数据结构决定的，这个顺序在外部看来可能没有明显的逻辑顺序。
- 随机性：随机性指的是元素的排列顺序或选择是由随机过程决定的

- 不可重复性指的是集合中不允许有重复的元素。如果尝试添加一个已经存在于集合中的元素，这个添加操作通常会被忽略，集合不会发生变化。例子：HashSet
- 原作者的不可重复性的那句话的意图是强调在Java中实现不可重复性集合（如Set）时，元素的唯一性是通过equals()方法来判断的。为了确保Set（尤其是基于哈希表的HashSet）的不可重复性能够正常工作，你需要正确地重写对象的equals()和hashCode()方法。

- 为什么需要重写equals()和hashCode()方法？
- equals()方法：用于判断两个对象是否相等。在Set中，当你尝试添加一个元素时，Set会使用equals()方法来检查是否已经有一个“相等”的元素存在。
- hashCode()方法：在使用基于哈希表的集合（如HashSet或HashMap）时，hashCode()方法用于计算对象的哈希码。哈希码用于在哈希表中快速定位对象。

- Java的合约：equals()与hashCode()
- Java规定，任何两个“相等”的对象必须具有相同的哈希码。这意味着，如果你重写了equals()方法来改变对象相等性的判断逻辑，你同样需要重写hashCode()方法来确保相等的对象有相同的哈希码。

- 这是因为基于哈希表的集合首先使用哈希码来定位元素的槽位（bucket），然后在这个槽位上使用equals()方法来检查所有潜在的匹配项。
...
`Queue` 扩展了 `Collection` 的接口，根据 **因为容量问题（队列没有足够的容量或队列为空）而导致操作失败后处理方式的不同** 可以分为两类方法: 一种在操作失败后会抛出异常，另一种则会返回特殊值。
...
- `ArrayDeque` 插入时可能存在扩容过程, 不过均摊后的插入操作依然为 O(1)。虽然 `LinkedList` 不需要扩容，但是每次插入数据时均需要申请新的堆空间，均摊性能相比更慢。**（PS：这道题比较的是他们在实现双端队列上的性能，只考虑在头部和尾部添加元素的情况；LinkedList申请新的堆空间的意思是，每次插入节点都需要创建一个新的节点对象，对象存放在堆中，所以新建对象需要申请新的堆空间）**
...
GPT：操作**阻塞**并不意味着操作不会执行，而是指在某些条件未得到满足时，执行该操作的线程会暂停执行，直到条件得到满足。
