...
Java解释器
Java解释器是JVM的一部分，它的主要职责是读取Java字节码，将每条字节码逐一解释为对应平台的机器指令并立即执行。这种逐条解释和执行的过程使得Java程序可以在任何安装了JVM的平台上运行。

即时编译器（JIT Compiler）
随着JVM的发展，为了提高性能，即时编译器被引入到JVM中。JIT编译器可以将热点代码（经常执行的代码）编译成本地机器码，并进行优化，从而提高程序的执行效率。一旦字节码被编译成机器码，它就可以直接在硬件上运行，这通常比逐条解释执行快得多。
...
- Java 的类是单继承的，C++ 支持多重继承；虽然 Java 的类不可以多继承，但是接口可以多继承。（PS(ChatGPT):单继承意味着一个类只能直接继承自一个父类；多继承意味着一个类可以直接继承自多个父类；虽然Java不支持类的多继承，但它支持接口的多继承）
...
`BigDecimal` 可以实现对浮点数的运算，不会造成精度丢失。通常情况下，大部分需要浮点数精确运算结果的业务场景（比如涉及到钱的场景）都是通过 `BigDecimal` 来做的。(PS(ChatGPT): BigDecimal严格意义上不能称作定点数。虽然BigDecimal在一定程度上类似于定点数，因为你可以指定小数点后的位数，但它更准确地说是一种可伸缩精度的十进制数。它的实际精度由其值和精度（可选的小数部分长度）决定，不像传统的定点数那样有固定的小数位数。）
