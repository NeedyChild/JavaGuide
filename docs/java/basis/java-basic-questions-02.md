...
主函数（main方法）
接口：
在Java 8之前，接口不能有main方法，因为接口不能有任何实现。
从Java 8开始，接口可以有静态方法，所以理论上你可以在接口中定义一个静态的main方法。但这通常不是常见的做法，因为接口主要是用来定义类型的行为，而不是作为应用程序的入口点。

抽象类：
抽象类可以有main方法。尽管你不能直接实例化一个抽象类，但main方法是静态的，可以不依赖对象实例被调用。因此，你可以在抽象类中定义main方法，并且可以运行它。


构造函数
接口：
接口不能有构造函数。接口是纯抽象的，它们不能被实例化，所以没有构造函数的概念。
接口定义了一种合同，它可以有默认方法（Java 8及以上版本），静态方法，但不能有构造函数。

抽象类：
抽象类可以有构造函数，尽管你不能直接实例化一个抽象类。
抽象类的构造函数在实例化其子类时被调用。子类构造函数可以通过super关键字显式地或隐式地调用抽象父类的构造函数。
...
**PS：ChatGPT、JavaGuide的JVM垃圾回收章节和Google搜到的博客都说字符串常量池存放的应该是字符串常量本身（而非引用），ChatGPT还说引用是保存在栈上的**
```java
// 在堆中创建字符串对象”ab“
...
1、如果字符串常量池中不存在字符串对象“abc”，那么它会在堆上创建两个字符串对象，其中一个字符串对象会被保存在字符串常量池中。
