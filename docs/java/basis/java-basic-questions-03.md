...
- **`Exception`** :程序本身可以处理的异常，可以通过 `catch` 来进行捕获处理。`Exception` 又可以分为 Checked Exception (受检查异常，必须处理) 和 Unchecked Exception (不受检查异常，可以不处理)。
- **`Error`**：`Error` 属于程序无法处理的错误 ，~~我们没办法通过 `catch` 来进行捕获~~不建议通过`catch`捕获处理 。例如 Java 虚拟机运行错误（`Virtual MachineError`）、虚拟机内存不够错误(`OutOfMemoryError`)、类定义错误（`NoClassDefFoundError`）等 。这些错误发生时，Java 虚拟机（JVM）一般会选择线程终止。
...
- `void printStackTrace()`: 在控制台上打印 `Throwable` 对象封装的异常信息（ChatGPT：具体包括异常的类型、消息和异常发生时的堆栈跟踪等信息）

### try-catch-finally 如何使用？

- `try`块：用于尝试捕获异常。其后可接零个或多个 `catch` 块，如果没有 `catch` 块，则必须跟一个 `finally` 块。
...
ChatGPT+网络查找+JavaGuide：

反射是Java的一种机制，让我们可以在运行时分析类以及执行类中方法：通过反射，我们可以在程序运行时动态创建对象，还能获取到类的信息（比如属性、构造器、方法等）。（我收藏的一篇阿里云博客）
...
1. 动态性、灵活性：反射机制允许代码在运行时访问和分析类，这意味着我们可以编写不必在编译时知道具体类的代码。

缺点：

1. 性能开销：反射操作通常比直接的代码执行要慢。

2. 安全性问题：反射可以用来访问私有成员和方法，这可能会违反封装原则，引入安全漏洞。（Google搜反射可否访问私有变量即可搜到，确实是可以的）

3. 可维护性：过度使用反射可能会使代码难以理解和维护。
