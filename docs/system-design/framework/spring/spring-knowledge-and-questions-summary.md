...
在Spring框架中，**依赖注入（Dependency Injection，DI）** 是一种核心技术，它允许实现控制反转（Inversion of Control，IoC）的概念。这个机制是通过一个IoC容器来实现的，该容器负责构造、配置和管理应用中的各种对象及其依赖关系。依赖注入主要用来减少代码的耦合度，增加程序的灵活性和可维护性。


**依赖注入的核心概念**

依赖关系：在编程中，当一个类A使用到另一个类B时，我们说A依赖于B。通常情况下，依赖关系会导致高耦合和代码维护难度。

控制反转（IoC）：IoC是一种设计原则，用于减少代码之间的耦合。通过控制反转，对象的创建和管理被反转给了框架或容器，而不是由对象或程序本身控制。

依赖注入：依赖注入是IoC的一种实现方式，它通过容器动态地将依赖关系注入到组件中。组件不需要自行创建或管理它们的依赖关系，而是由外部（容器）自动提供。


**依赖注入的类型：**
在Spring框架中，依赖注入主要有三种方式：

1.构造器注入：依赖通过组件的构造器传入。这种方法在创建对象时就提供了所有必需的依赖，是不可变的，并且确保了依赖的不可为空。

2.Setter注入：依赖通过公开的setter方法注入。这种方式使得依赖的设置更加灵活，并且可以在对象创建后的任何时间点修改依赖。

3.字段注入：依赖直接注入到组件的字段中。这是最简便的注入方式，但也被认为是最不推荐的，因为它违反了封装原则，使得单元测试变得困难。
